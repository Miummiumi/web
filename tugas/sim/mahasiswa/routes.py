...
def data_mahasiswa():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f'Akun -{form.npm.data} berhasil Terdaftar','info')
...
def edit_mahasiswa():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Data Barhasil Diubah!','warning')
...
def pengaduan():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Data Barhasil Ditambahkan','warning')
        return redirect(url_for('rmahasiswa.pengaduan'))
    return render_template('pengaduan.html', form=form, dt_pengaduan=dt_pengaduan)


@rmahasiswa.route("/updatepengaduan/<int:ed_id>/update", methods=['GET','POST'])
@login_required
def edt_pengaduan(ed_id): 
...
@rmahasiswa.route("/hapuspengaduan/<id>", methods=['GET','POST'])
@login_required
def hps_pengaduan(id):
...
@login_required
def ket_pengaduan(ed_id): 
    dt_pengaduan=Tpengaduan.query.get_or_404(ed_id)
    return render_template('ket_pengaduan.html', dt_pengaduan=dt_pengaduan)
